...
def query_data(query, behaviour, model_id, query_vectordb, connect_id):
    ...
    if behaviour in ['english', 'hindi', 'thai', 'spanish', 'french', 'german', 'bengali', 'tamil', 'arabic', 'italian']:
        ...
        prompt = 'You are a helpful AI assistant possessing vast knowledge and good reasoning skills. Be concise when answering questions'
    else:
        prompt = DEFAULT_PROMPT
    ...
    if (is_rag_enabled == 'yes' and query_vectordb == 'yes' and query is not None and len(query.split()) > 0 \
        and behaviour not in ['sentiment', 'pii', 'redact']) \
        or (is_rag_enabled == 'yes' and query_vectordb == 'yes' and behaviour == 'chat'):
        try:
            user_query = ''
            if behaviour == 'chat':
                # Maintains conversations over the socket
                chat_history_list = json.loads(base64.b64decode(query))
                if len(chat_history_list) > 0:
                    if 'Human' in chat_history_list[0]:
                       user_query = chat_history_list[0]['Human']
            else:
                # QnA
            ...
            return failure_response(connect_id, e)
...
def prepare_prompt_template(model_id, behaviour, prompt, context, query):
    ...
    if 'claude' in model_id:
        ...
        if behaviour == 'chat':
            ...
            prompt_template = claude3_prompt_builder_for_chat(prompt, chat_history_list, context)
...
def claude3_prompt_builder_for_chat(system_prompt, chat_history_list, context):
    prompt_content = []
    for chat_seq in chat_history_list:
        if 'Assistant' in chat_seq:
            prompt_content.append({"role": "assistant", "content": chat_seq['Assistant']})
        elif 'Human' in chat_seq:
            if context != '':
                user_prompt = f"""{context} 
                                Here is the user question <question>{chat_seq['Human']}</question> . """
            else:
                user_prompt = chat_seq['Human']
            prompt_content.append({"role": "user", "content": user_prompt})
    return { "anthropic_version": "bedrock-2023-05-31",    
             "max_tokens": 5000,
             "system": system_prompt,    
             "messages": prompt_content, "temperature": 0.1} 
